...
if __name__ == '__main__':
    ...
    for ID_i in range(len(AreaNumber)):
        ...
        for i in range(len(AreaNodeNumber)):
            ...
            print("i =", i)
            AreaDataSense[i]
            R_i_Sensor[0][AreaNodeNumber[i]] = AreaDataSense[i]
            # 一方面是感知数据，另一方面是传输数据到MD，所以有两倍的功耗，从论文中得知，每秒需要消耗的能量
            p_i_Sensor[0][AreaNodeNumber[i]] = AreaDataSense[i] * E_s * 2
        ...
        print("min u_i_j =", min(u_i_j[0][AreaNodeNumber]))
        print("max R_i_Sensor =", max(R_i_Sensor[0]))
        # np,show()
        # 每个节点收集的数据
        ...
        # 网络运行开始
        MinSensorEnger = min(E_Sensor[0][AreaNodeNumber])
        E_min = 0
        print("MinSensorEnger =", MinSensorEnger)
        # np,show()
        while E_min < MinSensorEnger:
            ...
            for i in range(len(Tour_L) - 2):
                ...
                # np,show()
                sensorList = GridNode[0][AreaCellLabel[i]]
                # print("sensorList =", sensorList)
                if len(sensorList) != 1:
                    print("子集中存在多个节点")
                    tc_temp = []
                    ...
                    for j in range(len(sensorList)):
                        ...
                        MDChargeEnergy = MDChargeEnergy + (
                                    (E_max - E_Sensor[0][sensorList[j]]) / u_i_j[0][sensorList[j]]) * u_i_j[0][
                                             sensorList[j]] / 0.3
                    ...
                    # MD为节点充电的能量等于，充电的时间*充电率*能量传输效率
                    MDChargeEnergy = MDChargeEnergy + ((E_max - E_Sensor[0][sensorList[0]]) / u_i_j[0][sensorList[0]]) * \
                                     u_i_j[0][sensorList[0]] / 0.3
                    # 等于MD收集sensorList[j]中数据所需的时间(其中的数据量从上一次接收后，到当前次收集为止的所有量)
                ...
                print("E_Sensor =", E_Sensor)
                print("R_Data =", R_Data)
                if min(E_Sensor[0][AreaNodeNumber]) < 0:
                    print("存在节点死亡，网络终止")
                    break
                # 2小时记录数据
                if 7200 - step < MD_Time < 7200 + step:
                    ...
                    if ID_i == 0:
                        ...
                        np.savetxt("2hourdata1.txt", hourdata, fmt="%d")
                    elif ID_i == 2:
                        np.savetxt("2hourdata2.txt", hourdata, fmt="%d")
                    elif ID_i == 3:
                        np.savetxt("2hourdata3.txt", hourdata, fmt="%d")
                # 4小时记录数据
                if 14400 - step < MD_Time < 14400 + step:
                    ...
                    if ID_i == 0:
                        ...
                        np.savetxt("4hourdata1.txt", hourdata, fmt="%d")
                    elif ID_i == 2:
                        np.savetxt("4hourdata2.txt", hourdata, fmt="%d")
                    elif ID_i == 3:
                        np.savetxt("4hourdata3.txt", hourdata, fmt="%d")
                # 6小时记录数据
                if 21600 - step < MD_Time < 21600 + step:
                    ...
                    if ID_i == 0:
                        ...
                        np.savetxt("6hourdata1.txt", hourdata, fmt="%d")
                    elif ID_i == 2:
                        np.savetxt("6hourdata2.txt", hourdata, fmt="%d")
                    elif ID_i == 3:
                        np.savetxt("6hourdata3.txt", hourdata, fmt="%d")
                # 8小时记录数据
                if 28800 - step < MD_Time < 28800 + step:
                    ...
                    if ID_i == 0:
                        ...
                        np.savetxt("8hourdata1.txt", hourdata, fmt="%d")
                    elif ID_i == 2:
                        np.savetxt("8hourdata2.txt", hourdata, fmt="%d")
                    elif ID_i == 3:
                        np.savetxt("8hourdata3.txt", hourdata, fmt="%d")
                # 10小时记录数据
                if 36000 - step < MD_Time < 36000 + step:
                    ...
                    if ID_i == 0:
                        ...
                        np.savetxt("10hourdata1.txt", hourdata, fmt="%d")
                    elif ID_i == 2:
                        np.savetxt("10hourdata2.txt", hourdata, fmt="%d")
                    elif ID_i == 3:
                        np.savetxt("10hourdata3.txt", hourdata, fmt="%d")
